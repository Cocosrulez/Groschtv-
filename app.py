...
# Dateipfad für den dauerhaften Speicher
SAVE_FILE = "sendeplan_speicher.json"

# Seitenkonfiguration für professionelles UX-Design
st.set_page_config(
    page_title="Master Control | TV-Programmdirektion", page_icon="📡", layout="wide"
)
...
# --- HILFSFUNKTIONEN FÜR DAUERHAFTEN SPEICHER ---
def lade_sendeplan():
  if os.path.exists(SAVE_FILE):
    try:
      ...
      pass
  # Standard-Fallback, falls noch keine Datei existiert
  return [
      {
...
def speichere_sendeplan(schedule_data):
  try:
...
if "master_schedule" not in st.session_state:
  st.session_state.master_schedule = lade_sendeplan()
...
st.markdown(
    "**Tagesgenaue Programmierung** — Sendeplan direkt bearbeiten mit"
    " automatischer Echtzeit-Logikprüfung."
)

col_kpi1, col_kpi2, col_kpi3 = st.columns(3)
with col_kpi1:
  st.metric(
      label="Eingetragene Sendeplätze",
      value=len(st.session_state.master_schedule),
  )
with col_kpi2:
  st.metric(label="Verfügbare Formate in DB", value=len(SERIES_DATABASE))
with col_kpi3:
  st.metric(label="Logik-Engine", value="Aktiv (Live-Korrektur & Auto-Save)")
...
with tab_builder:
  ...
  with st.form("primetime_builder_form"):
    ...
    with col_f1:
      target_day = st.selectbox(
          ...
          [
              ...
              "Samstag",
              "Sonntag",
              "Montag bis Freitag (Mo-Fr Serie)",
          ],
      )
    ...
    with col_f2:
      ...
      net_default_idx = (
          net_options.index(default_net)
          if default_net in net_options
          else 6
      )
      net_runtime = st.selectbox(
          "Netto-Laufzeit pro Folge (Min.)",
          options=net_options,
          index=net_default_idx,
      )

      ad_options = [0, 3, 5, 6, 8, 9, 10, 12, 15, 20]
      ad_default_idx = (
          ad_options.index(default_ad) if default_ad in ad_options else 3
      )
      ad_runtime = st.selectbox(
          "Werbezeit pro Folge (Min.)",
          options=ad_options,
          index=ad_default_idx,
      )

    total_duration = net_runtime + ad_runtime

    start_dt = datetime.combine(datetime.today(), start_time)
    calculated_end_dt = start_dt + timedelta(
        minutes=total_duration * episodes_count
    )

    if episodes_count > 1:
      st.info(
          f"💡 **Mehrfach-Planung aktiv:** Es werden **{episodes_count} Folgen**"
          f" eingeplant. Sende-Ende ca."
          f" **{calculated_end_dt.time().strftime('%H:%M')} Uhr**."
      )
    else:
      st.info(
          f"💡 **Sende-Fenster Vorschau:** Von"
          f" **{start_time.strftime('%H:%M')} Uhr** bis"
          f" **{calculated_end_dt.time().strftime('%H:%M')} Uhr**."
      )

    submitted = st.form_submit_button(
        "Sendung(en) in das tagesgenaue Schema aufnehmen"
    )
    ...
    if submitted:
      ...
      for i in range(episodes_count):
        ...
        if check_time_conflict(
            ...
            end_min,
            target_day,
            st.session_state.master_schedule + temp_entries,
        ):
          conflict_found = True
          break
        ...
        entry = {
            ...
            "Zeitfenster": (
                f"{ep_start_dt.strftime('%H:%M')} -"
                f" {ep_end_dt.strftime('%H:%M')}"
            ),
            "Sendung": show_name,
            "Staffel": season_num,
      ...
      if conflict_found:
        st.error(
            f"❌ **Sendeplatz-Kollision am {target_day}!** Überschneidung mit"
            " einem anderen Sendeplatz."
        )
      else:
        st.session_state.master_schedule.extend(temp_entries)
        # Direkt permanent auf der Festplatte abspeichern
        speichere_sendeplan(st.session_state.master_schedule)
        st.success(f"Erfolgreich für **{target_day}** eingepflegt & gespeichert!")
        st.rerun()

with tab_view:
  st.subheader("Wochen-Sendeplan & Live-Logikprüfung")
  st.markdown(
      "Du kannst die Tabelle hier direkt bearbeiten (z.B. das"
      " **Zeitfenster** anpassen, wenn ein Logikfehler angezeigt wird). Das"
      " System prüft die Werte in Echtzeit und sichert Änderungen"
      " automatisch!"
  )
  ...
  if len(st.session_state.master_schedule) > 0:
    ...
    day_order = {
        ...
        "Samstag": 6,
        "Sonntag": 7,
        "Montag bis Freitag (Mo-Fr Serie)": 0,
    }
    if "Tag / Wochentag" in df_plan.columns:
      df_plan["Sort"] = df_plan["Tag / Wochentag"].map(day_order).fillna(8)
      df_plan = df_plan.sort_values(by=["Sort", "Zeitfenster"]).drop(
          columns=["Sort"]
      )

    # Interaktiver Data Editor, damit der Nutzer direkt im Plan Änderungen vornehmen kann
    edited_df = st.data_editor(
        df_plan, use_container_width=True, key="schedule_editor", num_rows="dynamic"
    )

    # Wenn der Nutzer etwas in der Tabelle direkt geändert hat, aktualisieren und sofort speichern
    ...
    for idx, row in edited_df.iterrows():
      try:
        ...
        slot_duration = (e_parts[0] * 60 + e_parts[1]) - (
            s_parts[0] * 60 + s_parts[1]
        )

        if slot_duration != row["Gesamt (Min.)"]:
          logik_meldungen.append(
              f"⚠️ **Logikfehler bei '{row['Sendung']}' (Staffel"
              f" {row['Staffel']}, Ep. {row['Episode']}) am"
              f" {row['Tag / Wochentag']} ({row['Zeitfenster']}):** Das"
              f" Zeitfenster ist **{slot_duration} Min.** lang, aber die Sendung"
              " benötigt laut Laufzeit exakt **"
              f"{row['Gesamt (Min.)']} Min.**! *(Tipp: Passe das Zeitfenster"
              " oben direkt in der Tabelle an, um den Fehler zu beheben)*"
          )
      except Exception:
        logik_meldungen.append(
            f"⚠️ Ungültiges Zeitformat bei Zeile {idx+1}. Bitte im Format"
            " 'HH:MM - HH:MM' angeben."
        )
    ...
    if logik_meldungen:
      ...
      st.markdown("---")
      st.success(
          "✅ **Alles perfekt!** Alle Zeitfenster stimmen exakt mit den"
          " Programmdauern überein. Keine Logikfehler vorhanden."
      )
...
with tab_database:
  ...
  st.markdown(
      "Übersicht aller hinterlegten Formate mit echten Staffellimits, Netto-"
      " und Werbezeiten."
  )
